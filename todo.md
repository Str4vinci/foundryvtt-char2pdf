...
## Recently done
- [x] Removed 1,783 lines of unreachable renderer code from `generate_character_sheet.py` (6,002 -> 4,219 lines). Three superseded page renderers and their helpers had survived the move to the current `dnd-layout-*` renderer with no remaining callers: `render_sheet_v2` (plus `ensure_official_sheet_assets`, the `pdftoppm` background-image path), `render_cards` (plus `render_badges`, `render_stat_input`, `render_meta_tags`, `render_use_tracker`), and `render_sheet` (plus `_sheet_stylesheet` and the `_SHEET_STYLE_LEDGER` / `_SHEET_STYLE_GAZETTE` / `_SHEET_STYLE_CODEX` CSS blocks), along with the unused `normalize_hex_color`. Verified byte-identical output: 819 SHA-256 hashes over every theme x mode x paper x footer combination for two characters, plus the sheet context and the Fight Club XML export, are unchanged.
